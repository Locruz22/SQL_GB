...
CREATE TRIGGER from_users AFTER INSERT ON users
FOR EACH ROW
BEGIN INSERT INTO Logs VALUES (NULL, DEFAULT, "users", NEW.id, NEW.name);
END;

-- ----------------------------------------------------------------------

CREATE TRIGGER form_catalogs AFTER INSERT ON catalogs
FOR EACH ROW
BEGIN INSERT INTO Logs VALUES (NULL, DEFAULT, "catalogs", NEW.id, NEW.name);
END;

-- ----------------------------------------------------------------------

CREATE TRIGGER from_products AFTER INSERT ON products
FOR EACH ROW
BEGIN INSERT INTO Logs VALUES (NULL, DEFAULT, "products", NEW.id, NEW.name);
END;


# 2. (по желанию) Создайте SQL-запрос, который помещает в таблицу users миллион записей.

-- сам решить не смог, поэтому пришлось подсмотреть. Постарался разобрать запрос
...
INSERT INTO samples (name, birthday) VALUES
  ('Геннадий', '1990-10-05'),
  ('Наталья', '1984-11-12'),
...
-- самостоятельно сделать ДЗ не получилось, поэтому разобрал предствленный примеры.
